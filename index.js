...
function selectPatient(patientResult) {
	...
	rl.question('Select patient number: ', (answer) => {
	  	
	  	var selectPatientId = patientResult.length >= parseInt(answer) ? patientResult[(parseInt(answer)-1)].id : "67";
		console.log("\n");
	  	console.log("\n");
	  	console.log("If the there is an error with the patient we will use a default patient to test");

	  	encounterWebService.beginEncounterPatientAsync(selectPatientId).then(val => {
	  		console.log(val);
	  		let encounter = val;
	...
	rl.question('Type the name of a drug: ', (answer) => {
		...
		let availabilityInPharmacyIdsByGroupId = {"12":["123","124","127","128","125","126"],"1":["1","9","7","17","15","8","18","16","21"],"10":["19"],"13":["129","130"],"4":["76"],"11":["120"],"6":["20"]};
	  	drugWebService.findDrugsByFilterWithAvailabilityAsync(answer, availabilityInPharmacyIdsByGroupId, 24, "2").then(val => {
	      	...
	      	rl.question('Select a drug number: ', (answer) => {
	  			rl.close();
			  	var selectedDrug = drusgsByName.length >= parseInt(answer) ? drusgsByName[parseInt(answer) - 1] : null;

		  		manageAddedDrug(currentEncounter, selectedDrug);
			});
	  	...
	  	.catch(reason => {
			...
			manageAddedDrug(currentEncounter, null);
	    });
...
function manageAddedDrug(currentEncounter, selectedDrug) {
	console.log("\n");
  	console.log("\n");
  	console.log("If the there is an error with the drug we will use a default drug to test");
  	console.log("Now we get drug suggestions by drug id");

  	let drugId = selectedDrug != null ? selectedDrug.id : 7;

	drugWebService.getDrugSuggestionByDrugIdAsync([drugId], 70, 24).then(val => {
      	console.log(val);
	    ...
	    if (selectedDrug != null) {
	    	// We select the fisrt administration route by default but the user should decide
	    	var firstAdministrationRoute = selectedDrug.administrationRouteById[Object.keys(selectedDrug.administrationRouteById)[0]]

	    	currentEncounter.prescription = {
		         "references":[],
		         "nonDrugs":[],
		         "drugs":[],
		         "diagnostics":[],
		         ...
		         "consultCost":0.0,
		         "prescriptionCost":0.0,
		         "othersCost":0.0,
		         "patientPaymentDismissed":false,
		         "totalCost":0.0
		    };
		    currentEncounter.prescription.drugs = [{
		    	dose: 1,
			    doseUnitCode: "1",
			    allowsGenerics: false,
			    refills: 0,
			    drugDescription: selectedDrug.description,
			    drugId: selectedDrug.id,
			    vademecumId: selectedDrug.vademecumId,
			    vademecumDescription: selectedDrug.vademecumDescription,
			    duration: 1,
			    doseUnitDescription: selectedDrug.doseUnitSingularDescription,
			    administrationRouteDescription: firstAdministrationRoute.description,
			    frequencyTimeUnitCode: "2",
			    durationTimeUnitCode: "4",
			    administrationRouteId: firstAdministrationRoute.id,
			    frequency: 1,
			    prescriptionAbbreviatureCode: "1",
			    hours: [
			    	{
	                    "hour":8,
	                    "minutes":0
			    ...
			    drugDescriptionPhrase: selectedDrug.suggestedDescription
		    }];
		}
	    ...
	    encounterWebService.checkEncounterPrescriptionAsync(encounterPrescriptionCheck).then(val => {
	      	...
	      	patientWebService.getSameDayDrugTreatments(currentEncounter.patientId, "2020-09-07T18:56:30Z", [480], 1, "4", 1, "2").then(val => {
		      	console.log("getSameDayDrugTreatments ===>");
		      	console.log(val);
		      	reviewAndFinish(currentEncounter, encounterPrescriptionCheck.encounter.prescription.drugs[0]);
		    })
		  	.catch(reason => {
...
function reviewAndFinish(currentEncounter, addedDrug) {
  	console.log("\n");
	...
	var prescriptionDocumentRequest = {"drugs":[{"dose":1.0,"doseUnitCode":"1","allowsGenerics":true,"refills":0,"drugDescription":"ACEPRESS","drugId":"7","vademecumId":"2","vademecumDescription":"Vademecum Apolo","notes":"","notesToPharmacy":"","duration":1,"doseUnitDescription":"Tableta","administrationRouteDescription":"Oral","frequencyTimeUnitCode":"2","durationTimeUnitCode":"2","administrationRouteId":"10","frequency":1,"prescriptionAbbreviatureCode":"1","hours":[{"hour":8,"minutes":0}],"drugDescriptionPhrase":"ACEPRESS TABLETA 150 mg"}],"nonDrugs":[],"healthProblemReferences":[],"encounterId":"39808","forceSignature":false};
  	if (addedDrug != null) {
  		prescriptionDocumentRequest.drugs = [];
  		prescriptionDocumentRequest.drugs.push(addedDrug);
  	}
  	...
  	encounterWebService.reviewEncounter(prescriptionDocumentRequest).then(val => {
      	...
      	// If we reviewd the encounter, we should not send this value
      	delete currentEncounter.prescription;
